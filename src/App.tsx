  ...
  return (
    <div className="min-h-dvh bg-[#f3f3f1] text-black transition-colors duration-300 dark:bg-[#0d0d0d] dark:text-white">
          ...
          <section aria-label={ui.previewImage} aria-modal="true" className="relative z-10 flex max-h-full w-full max-w-5xl flex-col rounded-2xl border-[3px] border-black bg-white p-3 shadow-boo-2xl dark:border-white dark:bg-[#171717]" role="dialog">
            <div className="mb-3 flex items-center gap-2 px-1">
              ...
              <div aria-label={ui.zoomControls} className="flex shrink-0 items-center gap-1" role="group">
                <button aria-label={ui.zoomOut} className="grid size-9 place-items-center rounded-xl border-2 border-black bg-white text-lg font-black leading-none text-black shadow-boo-sm active:translate-x-0.5 active:translate-y-0.5 active:shadow-none disabled:cursor-not-allowed disabled:opacity-35 dark:border-white dark:bg-neutral-900 dark:text-white" disabled={previewZoom <= MIN_PREVIEW_ZOOM} onClick={() => changePreviewZoom(-PREVIEW_ZOOM_STEP)} type="button">−</button>
                <button aria-label={`${ui.resetZoom} (${Math.round(previewZoom * 100)}%)`} className="h-9 min-w-16 rounded-xl border-2 border-black bg-white px-2 text-xs font-black tabular-nums text-black shadow-boo-sm active:translate-x-0.5 active:translate-y-0.5 active:shadow-none dark:border-white dark:bg-neutral-900 dark:text-white" onClick={() => setPreviewZoom(1)} title={ui.resetZoom} type="button">{Math.round(previewZoom * 100)}%</button>
                <button aria-label={ui.zoomIn} className="grid size-9 place-items-center rounded-xl border-2 border-black bg-white text-lg font-black leading-none text-black shadow-boo-sm active:translate-x-0.5 active:translate-y-0.5 active:shadow-none disabled:cursor-not-allowed disabled:opacity-35 dark:border-white dark:bg-neutral-900 dark:text-white" disabled={previewZoom >= MAX_PREVIEW_ZOOM} onClick={() => changePreviewZoom(PREVIEW_ZOOM_STEP)} type="button">+</button>
              </div>
              <button aria-label={ui.closePreview} autoFocus className="grid size-9 shrink-0 place-items-center rounded-xl border-2 border-black bg-white text-black shadow-boo-sm active:translate-x-0.5 active:translate-y-0.5 active:shadow-none dark:border-white dark:bg-neutral-900 dark:text-white" onClick={closeImagePreview} type="button"><Icon name="x" className="size-4" /></button>
            </div>
            <div
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col border-r-2 border-black bg-white p-5 transition-transform duration-300 dark:border-white/20 dark:bg-[#141414] ${sidebarHidden ? 'lg:-translate-x-full' : 'lg:translate-x-0'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <button className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-black bg-black px-4 py-3 text-sm font-black text-white shadow-boo-inverse-lg active:translate-x-1 active:translate-y-1 active:shadow-none dark:border-white dark:bg-white dark:text-black" onClick={handleNewChat} type="button"><Icon name="plus" className="size-4" /> {ui.newChat}</button>
      ...
      <main className={`flex min-h-dvh flex-col transition-[padding] duration-300 ${sidebarHidden ? 'lg:pl-0' : 'lg:pl-[280px]'}`}>
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b-2 border-black/10 bg-[#f3f3f1]/90 px-4 backdrop-blur-xl dark:border-white/10 dark:bg-[#0d0d0d]/90 sm:px-7">
          <div className="flex items-center gap-3">
            {!sidebarOpen && <button aria-label={ui.openMenu} className="rounded-xl border-2 border-black bg-white p-2 shadow-boo-sm dark:border-white/30 dark:bg-neutral-900 lg:hidden" onClick={() => setSidebarOpen(true)} type="button"><Icon name="menu" /></button>}
            {sidebarHidden && <button aria-label={ui.showSidebar} className="hidden rounded-xl border-2 border-black bg-white p-2 shadow-boo-sm dark:border-white/30 dark:bg-neutral-900 lg:grid" onClick={() => setSidebarHidden(false)} type="button"><Icon name="menu" /></button>}
            <label className="relative inline-block"><span className="sr-only">{ui.selectModel}</span><select className="max-w-[135px] cursor-pointer appearance-none rounded-xl border-2 border-black bg-white py-2.5 pl-3 pr-9 text-xs font-black outline-none shadow-boo-md transition hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-boo-xl focus-visible:-translate-x-0.5 focus-visible:-translate-y-0.5 focus-visible:shadow-boo-xl active:translate-x-0.5 active:translate-y-0.5 active:shadow-none dark:border-white dark:bg-neutral-900 sm:max-w-[300px] sm:text-sm" onChange={(event) => setSelectedModel(event.target.value)} value={selectedModel}><option disabled value="">{models.length ? ui.chooseModel : ui.modelsUnavailable}</option>{models.map((model) => <option key={model.id} value={model.id}>{model.name} · {model.provider}</option>)}</select><Icon name="chevron-down" className="pointer-events-none absolute right-3 top-1/2 size-4 -translate-y-1/2" /></label>
          </div>
          <div className="flex items-center gap-1.5">
            <label className="relative inline-block"><span className="sr-only">{ui.language}</span><select aria-label={ui.language} className="w-[52px] cursor-pointer appearance-none rounded-xl border-2 border-black bg-white py-2.5 pl-2 pr-5 text-[10px] font-black outline-none shadow-boo-sm dark:border-white/30 dark:bg-neutral-900" onChange={(event) => setLanguage(event.target.value as AppLanguage)} value={language}><option value="en">EN</option><option value="id">ID</option></select><Icon name="chevron-down" className="pointer-events-none absolute right-1.5 top-1/2 size-3 -translate-y-1/2" /></label>
        ...
        <section className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-4 sm:px-8">
                ...
                return (
                  <article className={`flex gap-3 sm:gap-4 ${message.role === 'user' ? 'flex-row-reverse' : ''}`} key={message.id}>
                    ...
                    <div className={`flex max-w-[90%] flex-col sm:max-w-[78%] ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`w-fit max-w-full rounded-2xl border-2 p-3 text-sm font-medium leading-7 sm:p-4 sm:text-base ${message.role === 'user' ? 'border-black bg-black text-white shadow-boo-inverse-lg dark:border-white dark:bg-white dark:text-black' : 'border-black/20 bg-white dark:border-white/20 dark:bg-[#171717]'}`}>
                      ...
                      <div aria-label={`Aksi pesan ${message.role === 'assistant' ? 'Boo AI' : 'Anda'}`} className="mt-2 flex items-center gap-1 px-1" role="group">
                        <button aria-label={speakingMessageId === message.id ? 'Hentikan pembacaan pesan' : `Bacakan pesan ${message.role === 'assistant' ? 'Boo AI' : 'Anda'}`} aria-pressed={speakingMessageId === message.id} className={`grid size-7 place-items-center rounded-lg border transition disabled:cursor-not-allowed disabled:opacity-30 ${speakingMessageId === message.id ? 'border-red-600 bg-red-500 text-white shadow-boo-danger-sm' : 'border-black/15 bg-white text-neutral-500 hover:border-black hover:text-black dark:border-white/15 dark:bg-neutral-900 dark:hover:border-white dark:hover:text-white'}`} disabled={!isSpeechSupported || !message.content.trim()} onClick={() => handleToggleMessageSpeech(message.id, message.content)} title={speakingMessageId === message.id ? 'Hentikan pembacaan' : 'Bacakan pesan'} type="button"><Icon name={speakingMessageId === message.id ? 'stop' : 'volume'} className="size-3.5" /></button>
          ...
          <div className="sticky bottom-0 mx-auto w-full max-w-2xl bg-gradient-to-t from-[#f3f3f1] via-[#f3f3f1] to-transparent pb-2 pt-3 dark:from-[#0d0d0d] dark:via-[#0d0d0d] sm:pb-3">
            {(error || uploadError || referenceModelError) && <p className="mb-2 rounded-lg border-2 border-black bg-white px-3 py-1.5 text-xs font-bold shadow-boo-sm dark:border-white/30 dark:bg-neutral-900">{uploadError || referenceModelError || error}</p>}
            <form className="rounded-xl border-2 border-black bg-white p-1 shadow-boo-md transition focus-within:-translate-y-0.5 focus-within:shadow-boo-xl dark:border-white/35 dark:bg-[#171717]" onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-between px-0.5 pb-0.5">
                ...
                <button aria-label={isImageMode ? ui.createImage : ui.sendMessage} className="grid size-8 place-items-center rounded-md border-2 border-black bg-black text-white shadow-boo-inverse-sm disabled:cursor-not-allowed disabled:opacity-30 dark:border-white dark:bg-white dark:text-black" disabled={(!draft.trim() && !pendingImages.length) || Boolean(referenceModelError) || isLoading || !selectedModel} type="submit"><Icon name={isImageMode ? 'sparkles' : 'arrow-up'} className="size-3.5" /></button>
              </div>
            </form>
          </div>
        </section>
